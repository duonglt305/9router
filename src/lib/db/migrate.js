...
function importLegacyMain(adapter, data) {
  ...
  importWithAssertion(adapter, "apiKeys", data.apiKeys || [], (k) => {
    adapter.run(
      ...
      [k.id, k.key, k.name || null, k.machineId || null, k.allowedConnectionIds ? stringifyJson(k.allowedConnectionIds) : null, k.isActive === false ? 0 : 1, k.createdAt || new Date().toISOString()]
    );
